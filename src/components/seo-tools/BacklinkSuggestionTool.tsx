...
const BacklinkSuggestionTool: React.FC = () => {
  ...
  const generateBacklinkOpportunities = async () => {
    ...
    try {
      const prompt = `Generate 10 backlink opportunities for a ${contentType} website in the "${niche}" niche${website ? ` (website: ${website})` : ''}. 

      For each opportunity, provide:
      1. Website/platform name
      2. Type of backlink opportunity (Guest Post, Resource Page, Directory, Forum, etc.)
      3. Domain authority level (High, Medium, Low)
      4. Difficulty to obtain (Easy, Medium, Hard)
      5. Specific strategy to get the backlink
      6. How to find contact information

      Focus on realistic, high-quality opportunities that can help build domain authority.

      Format the response as JSON array with this structure:
      [
        {
          "website": "Industry Blog Name",
          "type": "Guest Post",
          "authority": "High",
          "difficulty": "Medium",
          "strategy": "Pitch a unique article idea related to your niche",
          "contactInfo": "Look for 'Write for Us' page or contact editor directly"
        }
      ]`;

      const content = await generateContent(prompt, 'brief');
