...
const KeywordSuggestionTool: React.FC = () => {
  ...
  const generateKeywords = async () => {
    ...
    try {
      const prompt = `Generate 15 long-tail keyword suggestions based on the seed keyword "${seedKeyword}"${industry ? ` in the ${industry} industry` : ''}. 

      For each keyword, provide:
      1. The keyword phrase
      2. Search intent (Informational, Commercial, Navigational, or Transactional)
      3. SEO difficulty (Low, Medium, or High)
      4. Estimated search volume range
      5. Competition level (Low, Medium, or High)

      Focus on long-tail keywords (3+ words) that are specific and have commercial potential.
