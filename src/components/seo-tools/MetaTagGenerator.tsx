...
const MetaTagGenerator: React.FC = () => {
  ...
  const generateMetaTags = async () => {
    ...
    try {
      const prompt = `Generate SEO-optimized meta tags for a ${businessType || 'website'} about "${topic}"${targetKeyword ? ` targeting the keyword "${targetKeyword}"` : ''}. 

      Please provide:
      1. A compelling meta title (50-60 characters)
      2. A persuasive meta description (150-160 characters)
